  ...
  return (
    <div>
      <PageHeader
        overline="Cohorts"
        title="Groups"
        subtitle="Trainee groups by course, teacher and institution."
          ...
          <motion.div key={g.id} data-testid={`group-card-${g.id}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: i * 0.05 }} whileHover={{ y: -4 }} onClick={() => navigate(`/dashboard/groups/${g.id}`)} className="group cursor-pointer rounded-lg border border-white/[0.07] bg-[#0A0A0B] p-6 transition-colors hover:border-[#B800FF]/30">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="font-display text-2xl font-light text-white">{g.students}<span className="text-sm text-zinc-600">/{g.limit}</span></p>
                <p className="text-[10px] uppercase tracking-widest text-zinc-600">Active</p>
              </div>
            </div>
            <h3 className="mt-5 font-display text-lg font-medium tracking-tight text-white">{g.name}</h3>
            <p className="mt-1 text-xs text-[#B800FF]">{g.course}</p>
            <p className="mt-2 flex items-center gap-2 text-xs text-zinc-500"><UserRound className="h-3.5 w-3.5" /> {g.teacher}</p>
            <p className="mt-0.5 flex items-center gap-2 text-xs text-zinc-500"><Building2 className="h-3.5 w-3.5" /> {g.institution}</p>
            <p className="mt-3 text-[11px] text-zinc-600">Created {new Date(g.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
          </motion.div>
        ))}
      </div>
    </div>
