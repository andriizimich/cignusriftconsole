...
export default function LessonDetail() {
  ...
  const endQuizzes = (l.quizzes || []).filter((q) => !q.block_id || q.block_id === "end");

  const QuizCard = ({ qz }) => (
    <div className="mt-3 rounded-md border border-[#0066FF]/25 bg-[#0066FF]/5 p-4" data-testid={`quiz-for-${qz.block_id}`}>
      <div className="flex items-start gap-2"><HelpCircle className="mt-0.5 h-4 w-4 text-[#0066FF]" /><p className="text-sm text-white">{qz.question}</p></div>
  ...
  const BlockRow = ({ b }) => {
    const qz = quizFor(b.id);
    return (
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="lg:col-span-1"><ContentBlockCard block={b} testid={`block-${b.id}`} /></div>
        <div className="lg:col-span-2">{qz ? <QuizCard qz={qz} /> : <div className="flex h-full items-center rounded-md border border-dashed border-white/10 px-4 py-3 text-xs text-zinc-600">No quiz attached to this block.</div>}</div>
      </div>
    );
  };
    ...
    <div>
      ...
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-3 flex items-center gap-2 text-sm text-zinc-500"><Clock className="h-4 w-4 text-zinc-600" /> {l.duration} min · {l.theory_blocks?.length || 0} theory · {l.practice_blocks?.length || 0} practice · {l.quizzes?.length || 0} quizzes</p>
        </div>
      ...
      <div className="space-y-6">
        <Widget testid="lesson-theory" title="Theory Content"><div className="space-y-5 p-6">{l.theory_blocks?.map((b) => <BlockRow key={b.id} b={b} />)}</div></Widget>
        <Widget testid="lesson-practice" title="Practice Content"><div className="space-y-5 p-6">{l.practice_blocks?.map((b) => <BlockRow key={b.id} b={b} />)}</div></Widget>
        {endQuizzes.length > 0 && (
          <Widget testid="lesson-final-quiz" title="Final Quizzes"><div className="p-6">{endQuizzes.map((qz, i) => <QuizCard key={i} qz={qz} />)}</div></Widget>
        )}
      </div>
    </div>
  );
